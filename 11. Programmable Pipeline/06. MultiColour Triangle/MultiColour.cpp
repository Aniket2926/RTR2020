...

{
  ...
  // create shader
	...
	glBindBuffer(GL_ARRAY_BUFFER, vbo_position); // color texture are kept here 
	...
	glGenBuffers(1, &vbo_color);					// Buffer to store vertex colors
	glBindBuffer(GL_ARRAY_BUFFER, vbo_color);		// Find that named object in memory
