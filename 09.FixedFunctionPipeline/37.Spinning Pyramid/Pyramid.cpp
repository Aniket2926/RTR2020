...
void initialize(void)
{
	...
	glShadeModel(GL_SMOOTH); //change for deapth....... accessories wala function
	
	glClearDepth(1.0f); // change for depth
	glEnable(GL_DEPTH_TEST); // chnage for depth
	glDepthFunc(GL_LEQUAL);// chnage for depth
	glHint(GL_PERSPECTIVE_CORRECTION_HINT, GL_NICEST);   //  Change for depth..perspective projection asta destrotion sathi use karaicha
...
void display(void)
{
	...
	static GLfloat angle = 0.0f;  //spining

	glClear(GL_COLOR_BUFFER_BIT|GL_DEPTH_BUFFER_BIT); //third call.......and change for depth and added depth buffer
	...
	glTranslatef(0.0f, 0.0f, -4.0f);   // 4th change for prespective 
