...
void Resize(int width, int height)
{
	...
	glMatrixMode(GL_PROJECTION);  // ortho change 
	glLoadIdentity();
	...
	if (width <= height)
	{
		glOrtho(-100.0f,  // left
			...
			-100.0f * ((GLfloat)height / (GLfloat)width),  //buttom
	...
	{
		glOrtho(-100.0f *((GLfloat)width / (GLfloat)height),  //dinominator //left
			100.0f*((GLfloat)width / (GLfloat)height),  // dinominator  //right
			-100.0f , // buttom
			100.0f , //top
			-100.0f, //near
...
void display(void)
{
	...
	glMatrixMode(GL_MODELVIEW); // ortho change next line too
	glLoadIdentity();

	glBegin(GL_TRIANGLES);
	glColor3f(1.0f, 0.0f, 0.0f);
	
	glVertex3f(0.0f, 50.0f, 0.0f); //ortho change 1 changed in 50
	...
	SwapBuffers(ghdc);// fourth call // dpoble buffer
