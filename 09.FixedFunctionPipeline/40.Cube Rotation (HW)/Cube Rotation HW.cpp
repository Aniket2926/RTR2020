...
void initialize(void)
{
	...
	pfd.cColorBits = 32;
	pfd.cDepthBits = 32; // change for 3D or depth
	pfd.cRedBits = 8;
	pfd.cGreenBits = 8;
	pfd.cBlueBits = 8;
	pfd.cAlphaBits = 8;
	...
	// depth or 3D function add on .... they have to be in order as below

	glShadeModel(GL_SMOOTH);
	glClearDepth(1.0f); // default 1 astoch
	glEnable(GL_DEPTH_TEST);
	glDepthFunc(GL_LEQUAL); // kon chota or kon maghe tyala kadun taak (HIDDEN SURFACE REMOVAL)
...
void display(void)
{
	...
	glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT); //  for deapth or 3D


	glMatrixMode(GL_MODELVIEW); //ortho prespective change 3rd change 

	glLoadIdentity();
	glTranslatef(0.0f, 0.0f, -5.0f);   // 4th change for prespective 

	//glRotatef(angle, 0.0f, 0.0f, 1.0f);  // Rutwik previous assingment madhe hee call use kele hote
	//glRotatef(angle, 1.0f, 0.0f, 0.0f);
	//glRotatef(angle, 0.0f, 1.0f, 0.0f);

	glRotatef(angle, 1.0f, 1.0f, 1.0f);  // HW 
	...
	glVertex3f(1.0f, -1.0f, 1.0f);//  left buttom

	glVertex3f(1.0f, -1.0f, -1.0f);// right buttom
